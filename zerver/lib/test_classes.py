...
class EmailChangeTestMixin(ZulipTestCase):
    ...
    A function `do_email_change` is defined in the subclasses to
    authenticate based on the authentication flow.
    '''
    __unittest_skip__ = True

    def create_email_change_confirmation_key(self, user_profile: UserProfile, new_email: str,
    ...
    def verify_user_email_changed(self, action_key: str, realm: Realm,
                                  email_address_visibility_public: bool=False) -> None:
    ...
    def test_email_change_success(self) -> None:
        ...
        result = self.do_email_change(user_profile, action_key)
    ...
    def test_email_change_delivery_email_public(self) -> None:
        ...
        self.verify_user_email_changed(action_key, user_profile.realm,
                                       email_address_visibility_public=True)
    ...
    def test_email_change_invalid_key(self) -> None:
        ...
        result = self.do_email_change(user_profile, 'invalid_action_key')
        self.assert_in_success_response(["Whoops. The confirmation link is malformed."],
                                        result)
    ...
    def test_social_auth_email_change_unregistered_user_login(self) -> None:
        ...
        # UserProfile is created here(not stored to db) as `do_change_email`
        # expects a UserProfile
        unregistered_user_profile = create_user_profile(get_realm('zulip'), email='new_email@zulip.com',
                                                        password=None, active=False, bot_type=None,
                                                        full_name='Nonexistent User', bot_owner=None,
                                                        is_mirror_dummy=False, tos_version=None,
                                                        timezone=None)
        user_profile = self.example_user('hamlet')
        new_email = 'new-hamlet'
        action_key = self.create_email_change_confirmation_key(user_profile, new_email)

        result = self.do_email_change(unregistered_user_profile, action_key)
