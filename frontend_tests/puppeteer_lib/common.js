...
class CommonUtils {
    ...
    async check_form_contents(page, form_selector, params) {
        for (const name of Object.keys(params)) {
            ...
            if (typeof expected_value === "boolean") {
                assert.equal(await page.$eval(name_selector, (el) => el.checked), expected_value, 'Form content is not as expected.');
            } else {
                assert.equal(await page.$eval(name_selector, (el) => el.value), expected_value, 'Form content is not as expected.');
            }
        }
    }
    ...
    async get_stream_id(page, stream_name) {
        return await page.evaluate(
            (stream_name) => stream_data.get_stream_id(stream_name), stream_name);
    }
