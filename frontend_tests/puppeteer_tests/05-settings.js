...
async function test_change_password(page) {
    ...
    await page.waitForFunction(()=>{return $("#change_password_modal").attr("aria-hidden") === "true"})
}
...
async function test_alert_words_section(page) {
    // Could be flaky: might need wait till the alert words section opens.
...
async function test_default_language_setting(page) {
    ...
    // As we've opened settings page in German we language status will be german.
